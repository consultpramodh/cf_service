/************************************************************
 * CF ServiceOps — V2 Fact-Driven Orchestrator
 * Version: 0.3.0-candidate
 * Date: 2026-09-17
 *
 * PRINCIPLE
 * Business facts are state. Actions are derived on every wake-up.
 * One remote mutation boundary per processRequest() call.
 ************************************************************/
var CF = CF || {};
...
CF.V2Orchestrator = (function () {
  ...
  function stateFromNext_(next){
    var map={
      ...
      CUSTOMER_EMAIL_CONFIRMED:'SYNCING_CUSTOMER_CORE',
  ...
  function preview(requestId){
    ...
    return {
      ...
      canonicalState:stateFromNext_(shadow.nextRequiredFact),
      nextRequiredFact:shadow.nextRequiredFact||'',
      nextAction:core.nextAction||'',
      facts:shadow.facts||{},
      complete:shadow.complete===true,
